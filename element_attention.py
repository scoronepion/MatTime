...
class multi_heads_self_attention(nn.Module):
    def __init__(self, feature_dim=45, num_heads=1, dropout=0.0):
        ...
        self.layer_temperature = nn.Linear(feature_dim, 3)
    ...
    def forward(self, key, value, query):
        ...
        output = nn.functional.relu(self.layer_temperature(output))

        return output, attention

if __name__ == '__main__':
    raw, _ = read_element(noise=False, rare_element_scaler=2)
    ...
    y_train = torch.from_numpy(y_train).view(batch_size, -1, temperature_size).to(device)
    y_test = torch.from_numpy(y_test).view(batch_size, -1, temperature_size).to(device)
