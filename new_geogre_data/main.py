...
if __name__ =='__main__':
    ...
    if torch.cuda.is_available():
        ...
        fatigue_train = torch.from_numpy(y_train[:, 0]).view(-1, 1).to(device)
        fatigue_test = torch.from_numpy(y_test[:, 0]).view(-1, 1).to(device)

        tensile_train = torch.from_numpy(y_train[:, 1]).view(-1, 1).to(device)
        tensile_test = torch.from_numpy(y_test[:, 1]).view(-1, 1).to(device)

        fracture_train = torch.from_numpy(y_train[:, 2]).view(-1, 1).to(device)
        fracture_test = torch.from_numpy(y_test[:, 2]).view(-1, 1).to(device)

        hardness_train = torch.from_numpy(y_train[:, 3]).view(-1, 1).to(device)
        hardness_test = torch.from_numpy(y_test[:, 3]).view(-1, 1).to(device)
    else:
    ...
    for epoch in range(epoch_num):
        def closure():
            optimizer.zero_grad()
            loss, _, _, _, _ = model(x_train, fatigue_train, tensile_train, fracture_train, hardness_train)
            loss.backward()
        ...
        if epoch % 10 == 9:
            ...
            loss, pred1, pred2, pred3, pred4 = model(x_test, fatigue_test, tensile_test, fracture_test, hardness_test)
            print('loss: {}'.format(loss))

            r21 = r2_score(torch.squeeze(pred1.cpu()).detach().numpy(), torch.squeeze(fatigue_test.cpu()).detach().numpy())
            r22 = r2_score(torch.squeeze(pred2.cpu()).detach().numpy(), torch.squeeze(tensile_test.cpu()).detach().numpy())
            r23 = r2_score(torch.squeeze(pred3.cpu()).detach().numpy(), torch.squeeze(fracture_test.cpu()).detach().numpy())
            r24 = r2_score(torch.squeeze(pred4.cpu()).detach().numpy(), torch.squeeze(hardness_test.cpu()).detach().numpy())
            r2 = r21 + r22 + r23 + r24
