...
'''
CrossModel1，三个温度联合在一起，普遍都到了0.95左右；dmax单一路，效果不太好，大概只有0.87
'''

def read_data():
    raw = pd.read_csv('fulldmaxttt.csv').dropna()
...
class CrossModel1(nn.Module):
    def __init__(self, length):
        ...
        self.linear1_2 = nn.Linear(length, length)
        self.linear1_3 = nn.Linear(length, length)
        self.linear1_4 = nn.Linear(length, 1)
    ...
    def forward(self, input, y1, y2, y3, y4):
        ...
        # 统一描述：out_路编号_阶段编号
        # 1路，第一阶段
        out1, _ = self.attention1_1(input, input, input)

        # 1路，第二阶段
        # out1, _ = self.attention1_2(out1, out1, out1)
        
        # 1路，第三阶段
        out1_3 = nn.functional.relu(self.linear1_1(out1))
        out1_3 = nn.functional.relu(self.linear1_2(out1_3))
        out1_3 = nn.functional.relu(self.linear1_3(out1_3))
        out1_3 = nn.functional.relu(self.linear1_4(out1_3))
        ...
        out2_1, out3_1_a = self.cross_stitch1(out2_1, out3_1)
        out3_1_b, out4_1 = self.cross_stitch2(out3_1, out4_1)
        out3_1 = 0.5 * out3_1_a + 0.5 * out3_1_b
        # 2 3 4路，第二阶段
...
class CrossModel2(nn.Module):
    def __init__(self, length):
        super(CrossModel2, self).__init__()
        # 因为有四个任务，所以需要有四路模型，中间三个十字绣单元
        # 每一路都采用两个attention模块，最后接一个线形层转化为最后的目标值
        self.attention1_1 = multi_heads_self_attention(feature_dim=length)
        self.attention1_2 = multi_heads_self_attention(feature_dim=length)
...
if __name__ =='__main__':
    ...
    model = CrossModel2(length=x_train.size()[1])
    if torch.cuda.is_available():
        model.to(device)
    model.double()
